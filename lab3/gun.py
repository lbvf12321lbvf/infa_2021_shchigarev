    ...
    def check(self, obj, typ=''):
        ...
        if self.hy == 0:
            if (self.y + r) >= y >= (self.y - r) and self.x < x < self.x + self.hx:
                obj.y = self.y + (r + 1) * np.sign(obj.v_y)
                obj.v_y = - obj.v_y * self.k
                obj.v_x *= self.k
        if self.hx == 0:
            if (self.x + r * self.kx) >= x >= (self.x - r * self.kx) and self.y < y < self.y + self.hy:
                obj.x = self.x - (r * self.kx + 1) * np.sign(obj.v_x)
                obj.v_x = - obj.v_x * self.k
                obj.v_y *= self.k
...
t_rare = defaultdict(lambda: Target())
...
def new_game(n):
    global t_standard, screen1, balls, bullet, score
    for i in range(n):
    ...
    num = 0
    t_standard[0].live = 1
    while t_standard[0].live or balls:
        for b in balls:
            b.move(boards)
            for i in range(n):
            ...
            for i in range(n - 1):
                if b.hit_test(t_carrier[i]):
                    ...
                    t_carrier[i].hit()
                    successful_targets += 1
            if successful_targets == 3 * n - 2 + num:
                canv.bind('<Button-1>', '')
